...
function getLandlordSigner(
  packet: LeasePacket,
  userDni: string,
): Signer | undefined {
  return packet.signers.find(
    (signer) => signer.roleInLease === "landlord" && signer.dni === userDni,
  );
}
...
export default function ArrendadorDashboardPage() {
  const currentUser = useCurrentUser();
  const userDni = currentUser?.dni ?? "";

  const myPackets = getPackets()
    .filter((packet) => getLandlordSigner(packet, userDni))
    .sort(
      (a, b) =>
        ...
        {myPackets.length === 0 ? (
          ...
          <ul className="flex flex-col gap-4">
            {myPackets.map((packet) => {
              const signer = getLandlordSigner(packet, userDni)!;
