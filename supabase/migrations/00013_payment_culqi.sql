-- Migration: 00013_payment_culqi.sql
-- Expand the payments table for Culqi integration, add webhook events,
...
ALTER TABLE public.payments
  ...
  CHECK (status IN ('pending', 'prepared', 'processing', 'charging', 'requires_3ds', 'verifying', 'completed', 'failed', 'refunded'));
...
ALTER TABLE public.payments
  ...
  ADD COLUMN IF NOT EXISTS device_finger_print_id text,
  ADD COLUMN IF NOT EXISTS source_token_hash text,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_payments_active_per_packet
  ...
  WHERE status IN ('prepared', 'processing', 'charging', 'requires_3ds', 'verifying', 'completed');
...
BEGIN
  ...
  SELECT id, status, amount_centimos, currency, challenge_nonce, realtor_id
  INTO v_existing
  FROM public.payments
  WHERE packet_id = p_packet_id
    AND status IN ('prepared', 'processing', 'charging', 'requires_3ds', 'verifying', 'completed')
  FOR UPDATE;
  ...
  IF FOUND THEN
    ...
    v_result.challenge_nonce := v_existing.challenge_nonce;
  ...
  INSERT INTO public.payments (
    ...
    payment_provider, idempotency_key, challenge_nonce, updated_at
  ) VALUES (
    p_packet_id, p_realtor_id, p_amount_centimos / 100.0, p_amount_centimos,
    p_currency, 'prepared', 'culqi', p_idempotency_key, p_challenge_nonce, now()
  )
  ...
  RETURNING id, status, amount_centimos, currency, challenge_nonce
  INTO v_result.payment_id, v_result.existing_status, v_result.amount_centimos,
       v_result.currency, v_result.challenge_nonce;
...
REVOKE EXECUTE ON FUNCTION public.claim_payment_attempt(uuid, uuid, integer, text, text, text) FROM public;
GRANT EXECUTE ON FUNCTION public.claim_payment_attempt(uuid, uuid, integer, text, text, text) TO service_role;

-- =============================================================================
-- 7. RPC: claim_charging
-- =============================================================================

DO $$ BEGIN
  CREATE TYPE public.claim_charging_result AS (
    payment_id uuid,
    packet_id uuid,
    amount_centimos integer,
    currency text,
    device_finger_print_id text,
...
CREATE OR REPLACE FUNCTION public.claim_charging(
  p_payment_id uuid,
  p_realtor_id uuid,
  p_from_status text,
  p_device_finger_print_id text DEFAULT NULL,
  p_source_token_hash text DEFAULT NULL
)
RETURNS public.claim_charging_result
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_result public.claim_charging_result;
BEGIN
  UPDATE public.payments
  SET status = 'charging',
      device_finger_print_id = COALESCE(p_device_finger_print_id, device_finger_print_id),
      source_token_hash = COALESCE(p_source_token_hash, source_token_hash),
      updated_at = now()
  WHERE id = p_payment_id
    AND realtor_id = p_realtor_id
    AND status = p_from_status
  RETURNING id, packet_id, amount_centimos, currency,
            device_finger_print_id, source_token_hash, challenge_nonce
  INTO v_result;
...
REVOKE EXECUTE ON FUNCTION public.claim_charging(uuid, uuid, text, text, text) FROM public;
GRANT EXECUTE ON FUNCTION public.claim_charging(uuid, uuid, text, text, text) TO service_role;

-- =============================================================================
-- 8. RPC: process_payment_success
-- =============================================================================
...
CREATE OR REPLACE FUNCTION public.process_payment_success(
  p_payment_id uuid,
  p_charge_id text,
  p_charge_amount_centimos integer,
...
BEGIN
  SELECT id, packet_id, realtor_id, amount_centimos, currency, status, payment_provider_ref
  INTO v_payment
  FROM public.payments
  WHERE id = p_payment_id
  FOR UPDATE;
  ...
  IF v_payment.status = 'completed' THEN
    IF v_payment.payment_provider_ref = p_charge_id THEN
      v_result.outcome := 'already_completed_same_charge';
    ELSE
  ...
  IF v_payment.status NOT IN ('charging', 'verifying') THEN
    v_result.outcome := 'invalid_status';
    RETURN v_result;
  END IF;

  IF v_payment.amount_centimos != p_charge_amount_centimos THEN
    v_result.outcome := 'amount_mismatch';
    RETURN v_result;
  END IF;

  IF v_payment.currency != p_charge_currency THEN
  ...
  UPDATE public.payments SET
    status = 'completed',
    payment_provider_ref = p_charge_id,
...
REVOKE EXECUTE ON FUNCTION public.process_payment_success(uuid, text, integer, text, text, uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.process_payment_success(uuid, text, integer, text, text, uuid) TO service_role;

-- =============================================================================
-- 9. RPC: claim_webhook_processing
-- =============================================================================
